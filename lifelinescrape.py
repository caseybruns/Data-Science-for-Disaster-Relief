...
def scrape_google(tuple_list, lifeline_num, api_key):
    ...
    for search_term in search_term_list:
        ...
        print(f'pulled locations for {search_term}')
        time.sleep(2)
        def component_builder(tuple_list):
            def unpack(cell):
                for n in tuple_list:
...
food_potable_water = ['food banks', 'grocery store', 'supermarket', 'water utility company', 'water treatment plant', 'dams', 'Public works department', 'reservoirs']
water_infrastructure = ['water treatment plant', 'Sewage treatment plant', 'water mains']
evacuation = []
durable_goods = []
agriculture = ['farm']
...
alerts_warnings_messages = ['radio stations', 'television stations']
dispatch_911 = []
responder_commmunications = []
financial_services = ['banks']
# LIFELINE 6: TRANSPORTATION
highway = ['road side assist', 'towing']
mass_transit = ['buses', 'metro station','public transport','metro stations', 'bus stations']
...
def find_lifelines(city, api_key, print_statments=True,
...
hazardous_incident=hazardous_incident):
    ...
    print_statments : default is True. If set to False, the print statements for each search term will not print.
    ...
    def scrape_google(complete_list, api_key):
        ...
        return df.to_csv(f'datasets/{city}.csv', index=False)
